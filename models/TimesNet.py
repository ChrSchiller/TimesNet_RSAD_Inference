...
class Model(nn.Module):
    ...
    def anomaly_detection(self, x_enc): # add x_mark here
        # Normalization from Non-stationary Transformer
        means = x_enc.mean(1, keepdim=True).detach()
        x_enc = x_enc - means
        stdev = torch.sqrt(
        ...
        ### note that there is no normalization of x_mark (= DOY) in neither of the temporal embeddings
        ### while this makes sense for nn.Embedding, we need to think about it for sin cos embeddings

        # embedding
        ### since no x_mark_enc is passed, the model will use the default value of None
        ### x_mark is only passed in forecast, imputation and classification
        ### but not in anomaly detection
        ### passing x_mark leads to explicit Time Embedding
        ### our goal is to pass x_mark!
        enc_out = self.enc_embedding(x_enc, None)  # [B,T,C] # add x_mark instead of None
        # TimesNet
        for i in range(self.layer):
            enc_out = self.layer_norm(self.model[i](enc_out))
        # project back
